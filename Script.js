...
function setupTypewriter() {
  ...
  // Mảng chứa nội dung bạn muốn hiển thị
  const phrases = [
    "TH Labs cung cấp giải pháp R&D trọn gói: Thiết kế, gia công In 3D/CNC đến lập trình hệ thống IoT, AI & Web App.",
    "Chúng tôi hiện thực hóa mọi ý tưởng công nghệ của bạn một cách chính xác và tối ưu nhất.",
  ];

  let phraseIndex = 0;
  let charIndex = 0;
  let isDeleting = false;
  let typeSpeed = 50; // Tốc độ gõ (càng nhỏ càng nhanh)
  ...
  function typeEffect() {
    ...
    if (isDeleting) {
      ...
      charIndex--;
      typeSpeed = 20; // Tốc độ xóa nhanh hơn tốc độ gõ
    } else {
      // Đang gõ: tăng ký tự
      textElement.textContent = currentPhrase.substring(0, charIndex + 1);
      charIndex++;
      typeSpeed = 60; // Tốc độ gõ bình thường
    }

    // Logic chuyển đổi trạng thái
    if (!isDeleting && charIndex === currentPhrase.length) {
      // Gõ xong câu -> Dừng lại để đọc -> Chuyển sang xóa
      isDeleting = true;
      typeSpeed = 3000; // Dừng 3 giây cho khách đọc kịp câu dài
    } else if (isDeleting && charIndex === 0) {
      // Xóa xong -> Chuyển sang câu tiếp theo -> Chuyển sang gõ
      isDeleting = false;
      phraseIndex = (phraseIndex + 1) % phrases.length; // Vòng lặp vô tận
